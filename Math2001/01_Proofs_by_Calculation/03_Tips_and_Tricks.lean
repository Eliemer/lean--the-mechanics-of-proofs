...
example {w : ℚ} (h1 : 3 * w + 1 = 4) : w = 1 :=
  calc
    w = (3 * w) / 3 := by ring -- w * 3/3
    _ = (3 * w + 1) / 3 - 1/3 := by ring -- w + (1/3 - 1/3)
    _ = 4/3 - 1/3 := by rw [h1]
    _ = 1 := by ring
...
example {x : ℤ} (h1 : 2 * x + 3 = x) : x = -3 :=
  calc
    x = 2 * x - x := by ring -- x + (x - x)
    _ = (2 * x + 3) - x - 3 := by ring -- x + (3 - 3)
    _ = x - x - 3 := by rw [h1]
    _ = -3 := by ring
...
example {x y : ℤ} (h1 : 2 * x - y = 4) (h2 : y - x + 1 = 2) : x = 5 :=
  calc
    x = 2 * x - x := by ring -- x + (x - x)
    _ = 2 * x - y - x + y := by ring -- x + (y - y)
    _ = 4 - x + y := by rw [h1]
    _ = 4 + (y - x + 1) - 1 := by ring -- x + (1 - 1)
    _ = 4 + 2 - 1 := by rw [h2]
    _ = 5 := by ring
...
example {x y : ℝ} (h1 : x + y = 4) (h2 : 5 * x - 3 * y = 4) : x = 2 :=
  calc
    x = (8*x + 3*y - 3*y) / 8 := by ring -- x * 8/8 + (3y-3y)
...
example {a b : ℚ} (h1 : a - 3 = 2 * b) : a ^ 2 - a + 3 = 4 * b ^ 2 + 10 * b + 9 :=
  calc
    a ^ 2 - a + 3 = ((a ^ 2 - 6 * a + 9) + 6 * a - 9) - a + 3 := by ring -- a ^ 2 = (a - 3) ^2 + 6*a - 9
    _ = (a - 3) ^ 2 + 6 * a - 9 - a + 3 := by ring
    _ = (a - 3) ^ 2 + 5 * a - 6 := by ring
    _ = (a - 3) ^ 2 + 5 * (a - 3 + 3) - 6 := by ring -- a = a + (3 - 3)
...
example {a b c : ℝ} (h1 : a + 2 * b + 3 * c = 7) (h2 : b + 2 * c = 3)
  ...
  calc
    ...
    _ = 7 + (b + 2 * c) - 3 * b - 5 * c := by ring
    _ = 7 + 3 - 3 * b - 5 * c := by rw [h2]
    _ = 10 - 3 *(b + 2 * c) + c := by ring
    _ = 10 - 3 * 3 + 1 := by rw [h2, h3]
    _ = 2 := by ring
